...
namespace std
{
	...
	class threadpool
	{
		...
		vector<std::tuple<std::wstring, thread>> _pool;	//线程池
		queue<std::tuple<Task, std::wstring>> _tasks;	// 任务队列,任务名称
		mutex _lock;                   //任务队列同步锁
	...
	public:
		...
		inline ~threadpool()
		{
			...
			for (auto& thread : _pool) {
				...
				if (std::get<1>(thread).joinable())
					std::get<1>(thread).join(); // 等待任务结束， 前提：线程一定会执行完
			}
		}
	public:
		// 提交一个任务
		...
		auto commit(const wchar_t* name, F&& f, Args&&... args) -> future<decltype(f(args...))>
		{
			if (!_run)    // stoped ??
		...
		// 提交一个无参任务, 且无返回值
		template <class F>
		void commit2(const wchar_t* name, F&& task)
		{
			if (!_run) return;
			{
				lock_guard<mutex> lock{ _lock };
				_tasks.emplace(std::forward<F>(task), name);
			}
	#ifdef THREADPOOL_AUTO_GROW
			if (_idlThrNum < 1 && _pool.size() < THREADPOOL_MAX_NUM)
				addThread(1);
	...
	private:
		...
		void addThread(unsigned short size)
		{
			...
			{   //增加线程数量,但不超过 预定义数量 THREADPOOL_MAX_NUM
				_pool.emplace_back(L"", [this](){ //工作线程函数
					while (true) //防止 _run==false 时立即结束,此时任务队列可能不为空
					{
						Task task; // 获取一个待执行的 task
						{
							...
							if (!_run && _tasks.empty())
								return;
							_idlThrNum--;
							auto ta = move(_tasks.front()); // 按先进先出从队列取一个 task

							task = std::get<0>(ta);
							auto name = std::get<1>(ta);

							auto id = std::this_thread::get_id();

							for (auto& it : _pool) {
								if (std::get<1>(it).get_id() == id) {
									std::get<0>(it) = name;
									break;
								}
							}
							...
							// KF_INFO(L"add task, task count: %d, idle count: %d, thread running info: %s",
